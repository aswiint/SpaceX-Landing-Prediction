...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                # The default select valuae is for ALL sites
...
def get_pie_chart(entered_site):
    ...
    if entered_site == "ALL":
        fig = px.pie(success_df, values="class", names="Launch Site", title="Success Pie Chart")
    else:
        print(filtered_df[["class","Launch Site"]])
        fig = px.pie(filtered_df, values="Launch Site", names="class", title="Success Pie Chart at " + str(entered_site))
...
def get_payload_scatter_chart(entered_site, payload_slider):
    ...
    if entered_site == "ALL":
        fig = px.scatter(spacex_df, x="Payload Mass (kg)", y="class", color="Booster Version",
                         title="Success Scatter Plot")
        fig.update_layout(xaxis_range=payload_slider)

    else:
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version",
                     title="Success Scatter Plot " + str(entered_site))
        fig.update_layout(xaxis_range=payload_slider)
